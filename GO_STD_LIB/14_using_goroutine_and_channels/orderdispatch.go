...
func DispatchOrdersRestricted(channel chan<- DispatchNotification) {
	rand.Seed(time.Now().UTC().UnixNano())
	orderCount := rand.Intn(3) + 2
	fmt.Println("Order count:", orderCount)
	for i := 0; i < orderCount; i++ {
		...
		// below code will start giving errors
...
func DispatchOrdersSelect(channel chan<- DispatchNotification) {
	rand.Seed(time.Now().UTC().UnixNano())
